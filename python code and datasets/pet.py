...
def predict():
    global cnn_model, labels, panel  # Ensure the panel exists for displaying the image
    ...
    if not ret:
        text.insert(END, "Error: Could not capture image from webcam\n")
        return  # Exit function if capture failed

    # Resize for CNN model
    img_resized = cv2.resize(img, (32, 32))
    img_array = np.array(img_resized).reshape(1, 32, 32, 3).astype('float32') / 255.0

    # Predict using CNN model
    preds = cnn_model.predict(img_array)
    predict_idx = np.argmax(preds)

    # Load feeding details
    data = f"Pet Detected: {labels[predict_idx]}\n"
    try:
        with open(f"food/{labels[predict_idx]}.txt", "r", encoding='utf-8') as file:
            data += file.read()
    except FileNotFoundError:
        data += f"\nNo feeding information found for {labels[predict_idx]}"
    ...
    img_pil = img_pil.resize((300, 300), Image.LANCZOS)  # Resize for GUI display
    img_tk = ImageTk.PhotoImage(img_pil)

    # If panel exists, update it; otherwise, create it
    if panel is None:
